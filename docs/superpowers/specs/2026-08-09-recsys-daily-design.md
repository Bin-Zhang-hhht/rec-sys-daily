...
| 内容类型 | 每日数量 | 规则 |
| --- | ---: | --- |
| 学术论文 | 目标 10 篇 | 优先新论文；不足时扩大候选时间窗，但不填充低相关或重复论文 |
| 技术博客 | 最多 5 篇 | 允许为 0；只选择高相关内容，不为凑数降低阈值 |

因此每日页面最多展示 15 条新推荐。

论文候选默认取最近 36 小时；若不足 10 篇，则逐步扩大到最近 7 天，并优先选择尚未出现在历史日报中的论文。仍不足时允许少于 10 篇，并在运行报告中记录原因。
...
日更默认只把排名最高的约 20 篇论文候选和 10 篇博客候选交给 LLM。LLM 在一次结构化输出中同时给出相关性、中文一句话摘要、标签、图谱关系和证据，避免为同一条内容多次调用模型。
...
冷启动 150 条内容按每批 8 条调用 LLM，通常约 19 次调用；日更最多处理约 30 条 shortlist，通常不超过 4 次调用。
