...
每日数据流水线和网站发布以北京时间每天 02:22 为目标执行时间。GitHub Actions 可能排队延迟，
因此 02:22 是调度目标而不是网站完成更新的分钟级承诺。
...
- **详情页**：展示 summary、结构化方法/结果/局限、证据定位、标签、相邻内容和原文链接。
  论文和博客均在话题标签下方的分割线后提供横向铺满标题区域的“阅读原文”按钮，沿用“在图谱中查看”的白底灰边样式，点击区域至少 44px 高，在新标签页打开来源；正文底部保留原文入口。
- **归档页**：按日报日期浏览历史推荐。
