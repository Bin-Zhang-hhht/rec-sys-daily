...
| 飞书日报卡片 | 每个成功发布日 1 张 | 最多展示 3 篇论文和 3 篇博客，不用低质量内容补足 |
...
顺序分别展示最多 3 篇论文和 3 篇技术博客，保留原始标题并附中文 summary；模板中的按钮进入
完整日报和站内搜索。
...
飞书发送一张明确写明“今日暂无符合要求内容”的空日报卡片。任一生产或发布阶段失败、当天正式
数据尚未晋升、Secret 缺失时均跳过推送。通知发送失败只影响独立通知工作流，不回滚网站、
canonical data 或 `state.json`。
